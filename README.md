...
- **Merkle root placement**: The Poseidon2 Merkle root lives in two places: (1) baked into the word-commit circuit as a `global` constant — the verification key itself encodes the root, so a valid proof is impossible against a wrong word list, and (2) hardcoded in the contract for on-chain Merkle verification of public guesses. Changing the word list requires recompiling the circuit, regenerating the VK, and redeploying the contract.
